...
type ExecutorResponse struct {
	...
	// It is optional so existing plugins remain wire-compatible.
...
type ExecutorStreamChunk struct {
	...
	// Err reports a stream error associated with this chunk.
	Err error
	// Usage carries the latest complete provider token accounting. Plugins may
	// emit a usage-only chunk with an empty Payload; the host consumes it
	// without forwarding an empty downstream frame.
	Usage *UsageDetail `json:"usage,omitempty"`
}
