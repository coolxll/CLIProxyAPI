...
func TestBuildAPIKeyClientsCounts(t *testing.T) {
	cfg := &config.Config{
		...
		LingmaKey: []config.LingmaKey{{APIKey: "l1"}},
		OpenAICompatibility: []config.OpenAICompatibility{
			{APIKeyEntries: []config.OpenAICompatibilityAPIKey{{APIKey: "o1"}, {APIKey: "o2"}}},
		},
	}

	gemini, vertex, claude, codex, xai, compat, lingma := BuildAPIKeyClients(cfg)
	if gemini != 3 || vertex != 1 || claude != 1 || codex != 2 || xai != 1 || compat != 2 || lingma != 1 {
		t.Fatalf("unexpected counts: %d %d %d %d %d %d %d", gemini, vertex, claude, codex, xai, compat, lingma)
	}
}
