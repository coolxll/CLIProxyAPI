...
func hostCall[T any](h hostRPC, method string, request any) (T, error) {
	var zero T
	if h.call == nil {
		return zero, fmt.Errorf("OpenRouter host callback is unavailable")
	}
	rawRequest, errMarshal := json.Marshal(request)
	if errMarshal != nil {
		return zero, fmt.Errorf("marshal host request: %w", errMarshal)
	}
	...
	if !envelope.OK {
		...
		return zero, fmt.Errorf("host call failed with unknown error")
	}
