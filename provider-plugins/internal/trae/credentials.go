...
func refreshToken(host hostRPC, creds *credentials, apiHost string) error {
	...
	// Claim today's free checkin credits (4700 points) to prevent 4011 rate limit
	_ = claimCheckinCredits(host, creds.JWTToken, creds.DeviceID)

	return nil
}

// deriveStableDeviceID derives a stable, account-bound 16-digit device ID from user identity or JWT token.
// Binding device ID to user identity prevents Trae business code 9074 (frequent device switching rate limit).
func deriveStableDeviceID(jwtToken, userID string) string {
	identity := strings.TrimSpace(userID)
	if identity == "" {
		if uid, ok := userIDFromJWT(jwtToken); ok {
			identity = uid
		}
	}
	if identity == "" {
		identity = strings.TrimSpace(jwtToken)
	}
	if identity == "" {
		return defaultTraeID
	}
	...
	mod := num % 10000000000000000
	return fmt.Sprintf("%016d", mod)
}

// claimCheckinCredits claims daily free check-in credits (4700 points) to prevent/heal 4011 rate limit errors.
func claimCheckinCredits(host hostRPC, jwtToken, deviceID string) error {
	if jwtToken == "" {
	...
	headers.Set("Authorization", "Cloud-IDE-JWT "+jwtToken)
	headers.Set("Content-Type", "application/json")
	headers.Set("X-Device-Id", did)
	headers.Set("X-Device-Type", deviceType)
	headers.Set("X-Device-Brand", deviceBrand)
	resp, err := host.do(pluginapi.HTTPRequest{
		Method:  http.MethodPost,
	...
	if err != nil {
		return fmt.Errorf("claim checkin credits: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("claim checkin credits HTTP %d: %s", resp.StatusCode, safeUpstreamMessage(resp.Body))
	}
	return nil
}
