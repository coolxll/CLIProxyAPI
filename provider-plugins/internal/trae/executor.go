...
func (p *Plugin) openUpstreamStream(host hostRPC, creds credentials, build *traeRequestBuildResult) (hostHTTPStreamResponse, error) {
	...
	if upstream.StatusCode < http.StatusOK || upstream.StatusCode >= http.StatusMultipleChoices {
		...
		bodyStr := string(body)
		// Self-healing: if 4011 rate limit (credits not claimed today or depleted), claim daily credits and retry once
		if upstream.StatusCode == 4011 || strings.Contains(bodyStr, "4011") {
			if errClaim := claimCheckinCredits(host, creds.JWTToken, creds.DeviceID); errClaim == nil {
				retryUpstream, errRetry := host.doStream(httpReq)
				if errRetry == nil && retryUpstream.StatusCode >= http.StatusOK && retryUpstream.StatusCode < http.StatusMultipleChoices {
					return retryUpstream, nil
				}
				if errRetry == nil {
					defer host.closeHTTPStream(retryUpstream.StreamID)
				}
			}
		}
		return hostHTTPStreamResponse{}, traeStatusErr{code: upstream.StatusCode, msg: bodyStr}
	}
	return upstream, nil
}
