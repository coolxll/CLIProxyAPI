...
func invalidEmptyClaudeToolHistoryIDs(messages []gjson.Result, idx int) ([]string, bool) {
	...
	for i := idx + 1; i < len(messages) && i <= idx+2; i++ {
		if messageHasClaudeInputValidationToolError(messages[i], idSet) {
			return ids, true
		}
	}
	return nil, false
...
func messageHasClaudeInputValidationToolError(message gjson.Result, ids map[string]bool) bool {
