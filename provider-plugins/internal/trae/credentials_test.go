...
func TestRefreshTokenViaHostCallback(t *testing.T) {
	...
	host := func(method string, raw []byte) ([]byte, error) {
		...
		capturedURLs = append(capturedURLs, req.Request.URL)
	...
	for _, u := range capturedURLs {
		if strings.Contains(u, "/cloudide/api/v3/trae/oauth/ExchangeToken") {
			foundExchange = true
			break
		}
	}
	if !foundExchange {
		t.Errorf("expected ExchangeToken in URLs: %v", capturedURLs)
	}
