...
func firstUsageInt(node gjson.Result, paths ...string) int64 {
	for _, path := range paths {
		...
		if value := node.Get(path + "_total"); value.Exists() {
			return value.Int()
		}
	}
	return 0
...
func openAIUsageFromResult(result gjson.Result) openaiUsage {
	promptTokens := firstUsageInt(result, "prompt_tokens", "input_tokens")
	completionTokens := firstUsageInt(result, "completion_tokens", "output_tokens")
	totalTokens := firstUsageInt(result, "total_tokens")
	if totalTokens == 0 {
		totalTokens = promptTokens + completionTokens
	}

	cachedTokens := firstUsageInt(result,
		"prompt_tokens_details.cached_tokens",
		"input_tokens_details.cached_tokens",
		"cache_read_input_tokens",
		"cached_tokens",
	)
	cacheCreationTokens := firstUsageInt(result, "cache_creation_input_tokens")
	reasoningTokens := firstUsageInt(result,
	...
	if totalCached := cachedTokens + cacheCreationTokens; totalCached > 0 {
		usageData.PromptTokensDetails = &openaiPromptDetails{CachedTokens: totalCached}
	}
