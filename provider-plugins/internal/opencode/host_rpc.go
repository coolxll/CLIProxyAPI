...
func hostCall[T any](h hostRPC, method string, request any) (T, error) {
	var zero T
	if h.call == nil {
		return zero, fmt.Errorf("OpenCode host callback is unavailable")
	}
	rawRequest, errMarshal := json.Marshal(request)
	if errMarshal != nil {
		return zero, fmt.Errorf("encode OpenCode host request %s: %w", method, errMarshal)
	}
	...
	if errUnmarshal := json.Unmarshal(rawResponse, &envelope); errUnmarshal != nil {
		return zero, fmt.Errorf("decode OpenCode host response %s: %w", method, errUnmarshal)
	}
	if !envelope.OK {
		if envelope.Error != nil && strings.TrimSpace(envelope.Error.Message) != "" {
			return zero, fmt.Errorf("%s", envelope.Error.Message)
		}
		return zero, fmt.Errorf("OpenCode host request %s failed", method)
	}
	...
	if errDecode := json.Unmarshal(envelope.Result, &result); errDecode != nil {
		return zero, fmt.Errorf("decode OpenCode host result %s: %w", method, errDecode)
	}
	return result, nil
}
