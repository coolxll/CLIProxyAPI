...
func TestBuildAPIKeyClientsCounts(t *testing.T) {
	...
	gemini, vertex, claude, codex, compat, lingma := BuildAPIKeyClients(cfg)
	if gemini != 2 || vertex != 1 || claude != 1 || codex != 2 || compat != 2 || lingma != 1 {
		t.Fatalf("unexpected counts: %d %d %d %d %d %d", gemini, vertex, claude, codex, compat, lingma)
	}
}
